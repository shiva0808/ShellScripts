...
if [[ ${#} -lt 2 ]]
then
  echo "you have not provided adequate number of arguments, follow below"
  echo "${0} 'username' 'first name of user' ..[]"
  exit 1
fi
if [[ $((${#}%2)) -ne 0 ]]
then
echo "You have not provided correct pair of arguments, correct usage as below"
echo "${0} 'username' 'first name of user' ..[]"
exit 1
fi
while [[ ${#} -ne 0 ]]
